...
export default function Requests() {
  ...
  const handleProductChange = (
    ...
    setNewProduct((prev) => ({
      ...
      [name === "requestProduct"
        ...
        ? "price"
        : name]:
        name === "requestQuantity" || name === "requestPrice"
          ? Number(value)
          : value,
    }));
  };
  ...
  const totalPedido = products.reduce(
  ...
  const formatItens = (itens: string) => {
    return itens
      ...
      .replace(
        /(Total Pedido:.*?R\$ \d+\.\d{2})/g,
        '<div class="pt-4 text-xl font-bold">$1</div>'
      )
      .replace(/\n/g, "<br>");
  };
                    ...
                    >
                      ...
                      <td className="border border-black px-4 py-4 whitespace-nowrap">
                        <button
                          ...
                          onClick={() =>
                            openModal("Itens do Pedido", pedido.itens)
                          }
                        >
                          <Eye />
                      ...
                      <td className="border border-black px-4 py-4 whitespace-nowrap">
                        <button
                          className="text-blue-600 cursor-pointer relative group top-4 left-1/2 transform -translate-x-1/2 -translate-y-1/2"
                          onClick={() =>
                            openModal("Observações", pedido.observacoes)
                          }
                        >
                          <Eye />
                          <div className="absolute right-0 bottom-5 mb-2 hidden group-hover:block bg-black text-white text-xs rounded py-1 px-2">
              ...
              <div>
                ...
                {products.map((product, index) => (
                  <span key={index} className="mt-2 flex gap-3">
                    ...
                    <div>
                      <span className="font-semibold"> Preço Unitário:</span> R${" "}
                      {product.price}
                    </div>
                    <div>
