...
export default function Sidebar() {
  ...
  const handleLogout = async () => {
    try {
      ...
      if (data.success) {
        // apaga credenciais de "lembrar o acesso"
        localStorage.removeItem("remember");
        localStorage.removeItem("email");
        localStorage.removeItem("password");
        // limpa qualquer sessionStorage
      ...
      const msg = axios.isAxiosError(error)
        ...
        : String(error);
      console.error("Erro ao conectar com o servidor:", msg);
    }
  };
      ...
      <NavigationMenu.Root className="h-full w-full custom-scrollbar overflow-y-auto mb-10">
        <NavigationMenu.List className="h-full w-full p-2">
          {menuItems.map((tab, index) => {
            const showSeparator =
              menuItems[index]?.name === "Dashboard" ||
              menuItems[index]?.name === "Usuários";
