...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (password.length < MIN_PASSWORD_LENGTH) {
      setMessage(`A senha deve ter no mínimo ${MIN_PASSWORD_LENGTH} caracteres.`);
      return;
    }
  ...
  return (
    <Form.Root onSubmit={handleSubmit} className="space-y-6 font-montserrat">
      ...
      <Form.Submit asChild>
        <button
          type="submit"
          className="w-full bg-green-700 hover:bg-green-800 text-white font-bold py-2 px-4 rounded shadow transition duration-200 disabled:opacity-50 cursor-pointer disabled:cursor-not-allowed"
          disabled={loading}
        >
