...
function salvarLog($conn, $log_conteudo, $log_acao, $log_status = "sucesso") {
    ...
    $sql = "INSERT INTO log (log_user_id, log_user_nome, log_datahora, log_pag_id, log_url, log_acao, log_conteudo, log_ip, log_user_agent, log_status, log_referencia) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    $stmt = $conn->prepare($sql);
    $stmt->bind_param("issssssssss", $userData['user_id'], $log_user_nome, $log_datahora, $log_pag_id, $log_url, $log_acao, $log_conteudo, $log_ip, $log_user_agent, $log_status, $log_referencia);
    $stmt->execute();
    $stmt->close();
